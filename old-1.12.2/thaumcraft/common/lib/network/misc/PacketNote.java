...
public class PacketNote implements IMessage, IMessageHandler<PacketNote, IMessage> {
   ...
   public IMessage onMessage(PacketNote message, MessageContext ctx) {
      if (ctx.side == Side.CLIENT) {
         ...
         TileEntity tile = world.func_175625_s(new BlockPos(message.x, message.y, message.z));
         byte note = -1;
         if (tile != null && tile instanceof TileEntityNote) {
            note = ((TileEntityNote)tile).field_145879_a;
         } else if (tile != null && tile instanceof TileArcaneEar) {
            note = ((TileArcaneEar)tile).note;
         }

         if (note >= 0) {
            PacketHandler.INSTANCE.sendToAllAround(new PacketNote(message.x, message.y, message.z, message.dim, note), new TargetPoint(message.dim, (double)message.x, (double)message.y, (double)message.z, 8.0D));
         }
      }
