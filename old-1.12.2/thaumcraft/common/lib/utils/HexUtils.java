...
public class HexUtils {
   ...
   public static ArrayList<HexUtils.Hex> getRing(int radius) {
   ...
   public static HashMap<String, HexUtils.Hex> generateHexes(int radius) {
   ...
   public static class Hex {
      ...
      public static HexUtils.Hex fromString(String hs) {
         String[] ss = hs.split(":");
         if (ss.length == 2) {
            try {
               int q = Integer.parseInt(ss[0]);
               int r = Integer.parseInt(ss[1]);
               return new HexUtils.Hex(q, r);
