...
public class WavefrontObject implements IModelCustom {
   ...
   private Vertex parseVertex(String line, int lineCount) throws WavefrontObject.ModelFormatException {
      ...
      if (isValidVertexLine(line)) {
         ...
         try {
            ...
            throw new WavefrontObject.ModelFormatException(String.format("Number formatting error at line %d", lineCount), var6);
         }
      } else {
   ...
   private WavefrontObject.TextureCoordinate parseTextureCoordinate(String line, int lineCount) throws WavefrontObject.ModelFormatException {
      ...
      if (isValidTextureCoordinateLine(line)) {
         ...
         try {
            if (tokens.length == 2) {
               ...
               return (WavefrontObject.TextureCoordinate)(tokens.length == 3 ? new WavefrontObject.TextureCoordinate(Float.parseFloat(tokens[0]), 1.0F - Float.parseFloat(tokens[1]), Float.parseFloat(tokens[2])) : textureCoordinate);
            }
         } catch (NumberFormatException var6) {
            throw new WavefrontObject.ModelFormatException(String.format("Number formatting error at line %d", lineCount), var6);
         }
   ...
   private WavefrontObject.Face parseFace(String line, int lineCount) throws WavefrontObject.ModelFormatException {
      WavefrontObject.Face face = null;
      if (isValidFaceLine(line)) {
         face = new WavefrontObject.Face();
         ...
         if (tokens.length == 3) {
            if (this.currentGroupObject.glDrawingMode == -1) {
               ...
               throw new WavefrontObject.ModelFormatException("Error parsing entry ('" + line + "', line " + lineCount + ") in file '" + this.fileName + "' - Invalid number of points for face (expected 4, found " + tokens.length + ")");
            }
         } else if (tokens.length == 4) {
            if (this.currentGroupObject.glDrawingMode == -1) {
               this.currentGroupObject.glDrawingMode = 7;
            } else if (this.currentGroupObject.glDrawingMode != 7) {
               throw new WavefrontObject.ModelFormatException("Error parsing entry ('" + line + "', line " + lineCount + ") in file '" + this.fileName + "' - Invalid number of points for face (expected 3, found " + tokens.length + ")");
            }
         }
         ...
         if (isValidFace_V_VT_VN_Line(line)) {
            ...
            face.textureCoordinates = new WavefrontObject.TextureCoordinate[tokens.length];
            face.vertexNormals = new Vertex[tokens.length];

            for(i = 0; i < tokens.length; ++i) {
               subTokens = tokens[i].split("/");
               face.vertices[i] = (Vertex)this.vertices.get(Integer.parseInt(subTokens[0]) - 1);
               face.textureCoordinates[i] = (WavefrontObject.TextureCoordinate)this.textureCoordinates.get(Integer.parseInt(subTokens[1]) - 1);
   ...
   private WavefrontObject.GroupObject parseGroupObject(String line, int lineCount) throws WavefrontObject.ModelFormatException {
      WavefrontObject.GroupObject group = null;
      if (isValidGroupObjectLine(line)) {
         String trimmedLine = line.substring(line.indexOf(" ") + 1);
         if (trimmedLine.length() > 0) {
            group = new WavefrontObject.GroupObject(trimmedLine);
         }
