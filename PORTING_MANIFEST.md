...
- `PORTING.md`
- `PORTING_MEMORY.md`
- Latest implementation checkpoint: see `PORTING.md` snapshot dated `2026-02-09` (includes spa mix-mode baseline, legacy-shaped warp cadence with gear/research-unlock hooks plus milestone-to-research migration, baseline research stage/flag/knowledge scaffolding + debug command tooling, thaumometer-driven observation/epiphany knowledge gain baseline, research-table persisted theory-session baseline (`Draft`/`Complete` loop) with synced menu/session counters, cultist/crimson warping armor baselines, vis-discount gear/workbench integration, restored workbench charger 3x3 aura behavior baseline, quartz-sliver catalyst migration to canonical `nugget_quartz` split-ID semantics with compatibility-tag preservation, legacy armor/goggle texture+item-model asset restoration with custom model-id datagen exclusions, second and third static-safe block-model migration batches moved from generated placeholders to custom main resources (including rails/ears/lamps/smelters/mirrors/research-table plus OBJ-backed wrappers for `arcane_workbench_charger`, `golem_builder`, `pillar`), and explicit tracking that player-worn armor/goggles geometry parity is still pending).
